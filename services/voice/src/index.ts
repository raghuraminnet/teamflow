...
app.get('/voice/stats/summary', requireAuth, async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT
         ...
         ROUND(AVG(CASE WHEN duration_secs > 0 THEN duration_secs END)) as avg_duration_secs
       FROM voice.call_logs`
    );
    res.json({
      data: {
        totalCalls: parseInt(rows[0].total_calls, 10),
        answered: parseInt(rows[0].answered, 10),
        missed: parseInt(rows[0].missed, 10),
        avgDurationSecs: rows[0].avg_duration_secs ? parseInt(rows[0].avg_duration_secs, 10) : 0,
      }
    });
  } catch (err) {
    res.status(500).json({ error: (err as Error).message });
...
async function main() {
  ...
  try {
    console.info('[Voice] Starting AMI connect, ami.connected=' + ami.connected);
    await ami.connect();
    console.info('[Voice] AMI connect() returned, ami.connected=' + ami.connected);
    console.info('[Voice] AMI connected');
  } catch (err) {
    console.warn('[Voice] AMI connect failed (will retry):', (err as Error).message);
    console.warn('[Voice] After fail, ami.connected=' + ami.connected);
  }

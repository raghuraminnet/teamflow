...
function makeRawProxy(targetHost: string, targetPort: number, fromPrefix: string, toPrefix: string) {
  return function(req: http.IncomingMessage, res: http.ServerResponse) {
    const rawUrl = (req.headers as Record<string, string>)['x-original-url'] ?? req.url ?? '';
    ...
    if (authHdr?.startsWith('Bearer ')) {
      try {
        ...
        (req.headers as Record<string, string>)['x-user-id'] = String(payload.userId);
        (req.headers as Record<string, string>)['x-user-role'] = payload.role;
        (req.headers as Record<string, string>)['x-user-email'] = payload.email ?? '';
      } catch {}
    }
    ...
    for (const [k, v] of Object.entries(req.headers)) {
      if (!v) continue;
      if (['host', 'connection', 'content-length', 'content-encoding', 'transfer-encoding', 'x-original-url'].includes(k)) continue;
    ...
    if (req.method === 'GET' || req.method === 'HEAD') {
      ...
      proxyReq.on('error', (err) => { if (!res.writableEnded) { res.statusCode = 502; res.end(); } console.error(`[proxy] err:`, err.message); });
      proxyReq.on('timeout', () => { proxyReq.destroy(); if (!res.writableEnded) { res.statusCode = 504; res.end(); } console.error(`[proxy] timeout`); });
    ...
    proxyReq.on('timeout', () => {
      ...
      proxyReq.destroy();
      if (!res.writableEnded) { res.statusCode = 504; res.end(); }
    });
...
const expressHandler = app as unknown as http.RequestListener;
...
const server = http.createServer((req, res) => {
  ...
  const originalUrl = (req.headers as Record<string,string>)['x-original-url'] ?? req.url ?? '';

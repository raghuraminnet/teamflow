...
router.get('/', async (req, res) => {
    ...
    const conditions = [];
    if (projectId)
        conditions.push(eq(tasks.projectId, Number(projectId)));
    if (assignedTo)
        conditions.push(eq(tasks.assignedTo, Number(assignedTo)));
    if (status)
    ...
    const allUserIds = [...new Set(rows.map(r => [r.assignedTo, r.createdBy]).flat().filter(Boolean))];
    let userMap = {};

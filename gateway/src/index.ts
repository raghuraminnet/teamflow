...
// ── Other proxies ───────────────────────────────────────────────────────────
function buildProxyConfig(pathPrefix: string, targetHost: string, ws = false) {
  return {
    ...
    on: {
      proxyReq: (proxyReq: http.ClientRequest, req: Request) => {
        ...
        if (authReq.userId) proxyReq.setHeader('X-User-Id', authReq.userId);
        if (authReq.userRole) proxyReq.setHeader('X-User-Role', authReq.userRole);
        if (authReq.userEmail) proxyReq.setHeader('X-User-Email', authReq.userEmail);
      },
      error: (_err: Error, _req: Request, res: unknown) => {
        if (res && typeof res === 'object' && 'statusCode' in res) {
          (res as { statusCode: number }).statusCode = 502;
          (res as any).end?.();
        }
      },
    },
